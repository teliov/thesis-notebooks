...
def rf_inference(model_file, test_files, symptoms_db_json, output_dir, name=""):
    logger = report.Logger("Random Forest %s Inference on QCE" % name)
    ...
    try:
        logger.log("Starting Random Forest Inference")
        for test_name, test_csv in test_data.items():
            ...
            logger.log("Running RF %s Inference" % test_name)
            ...
            test_result_file = os.path.join(output_dir, "rf_%s_inference.json" % test_name)
            with open(test_result_file ,"w") as fp:
                json.dump(test_results, fp, indent=4)

            end = timer()
            logger.log("Completed %s Random Forest Inference: %.5f secs" % (test_name, end - start))
        res= True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Medvice RandomForest Inference Engine')
    ...
    rf_inference(model_file=model_file, test_files=test_files, symptoms_db_json=symptoms_db_json, output_dir=output_dir)
